...
struct ChatCommandRecord {
    ...
    var name ChatCommandTemplateName; // Class name of the invoked chat command
...
function bool IsChatCommandOnCooldown(X2TwitchChatCommandTemplate Command, string SenderLogin) {
    return !IsAllowedUnderLimits(Command.IndividualRateLimits, Command.DataName, SenderLogin, true)
        || !IsAllowedUnderLimits(Command.GlobalRateLimits,     Command.DataName, SenderLogin, false);
}

function RecordCommandUsage(X2TwitchChatCommandTemplate CommandInvoked, string SenderLogin) {
    local ChatCommandRecord Record;

    Record.SenderLogin = SenderLogin;
    Record.ChatCommandTemplateName = CommandInvoked.DataName;
...
protected function bool IsAllowedUnderLimits(ChatCommandRateLimitConfig LimitConfig, name ChatCommandTemplateName, string SenderLogin, bool IsIndividualConfig) {
    ...
    if (LimitConfig.CooldownInSeconds > 0) {
        ...
        for (Index = CommandHistory.Length - 1; Index >= 0; Index--) {
            ...
            if (CommandHistory[Index].ChatCommandTemplateName != ChatCommandTemplateName || (IsIndividualConfig && CommandHistory[Index].SenderLogin != SenderLogin)) {
                continue;
            }

            if (ElapsedTime < LimitConfig.CooldownInSeconds) {
                `TILOG("Viewer " $ SenderLogin $ " can't use chat command " $ ChatCommandTemplateName $ " because it's on cooldown for " $ (LimitConfig.CooldownInSeconds - ElapsedTime) $ " more seconds");
                return false;
            }
        }
    }
    ...
    if (LimitConfig.CooldownInTurns > 0) {
        for (Index = CommandHistory.Length - 1; Index >= 0; Index--) {
            ...
            if (ElapsedTurns < LimitConfig.CooldownInTurns) {
                `TILOG("Viewer " $ SenderLogin $ " can't use chat command " $ ChatCommandTemplateName $ " because it's on cooldown for " $ (LimitConfig.CooldownInTurns - ElapsedTurns) $ " more turns");
                return false;
            }
        }
    }
    ...
    if (LimitConfig.MaxUsesPerTurn > 0) {
        for (Index = CommandHistory.Length - 1; Index >= 0; Index--) {
            ...
            if (NumUsesThisTurn >= LimitConfig.MaxUsesPerTurn) {
                `TILOG("Viewer " $ SenderLogin $ " can't use chat command " $ ChatCommandTemplateName $ " because it's on cooldown due to max uses this turn being reached");
                return false;
            }
        }
    }
